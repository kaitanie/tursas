...
(defmulti hash-it (fn [repository _bytes]
...
(defmethod hash-object :blob [repository object]
  (cond (bytes? object) (hash-it repository object)
        (string? object) (hash-it repository (str->bytes object))
        :else            (hash-it repository (str->bytes (str object)))))
...
(defn make-sha1-map-repo []
  {:repository/hash-implementation :sha1
   :repository/storage-implementation :in-memory-map})
